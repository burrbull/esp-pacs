...
impl W {
    ...
    #[inline(always)]
    #[must_use]
    pub fn cam_conv_8bits_data_inv(
        &mut self,
    ) -> CAM_CONV_8BITS_DATA_INV_W<CAM_RGB_YUV_SPEC> {
        CAM_CONV_8BITS_DATA_INV_W::new(self, 21)
    }
    ///Bits 22:23 - In YUV-to-YUV mode, 0: data is converted to YUV422 format. 1: data is converted to YUV420 format. 2: data is converted to YUV411 format. 3: disabled. To enable YUV-to-YUV mode, LCD_CAM_CAM_CONV_TRANS_MODE must be set to 1.
    #[inline(always)]
    #[must_use]
    pub fn cam_conv_yuv2yuv_mode(
        &mut self,
    ) -> CAM_CONV_YUV2YUV_MODE_W<CAM_RGB_YUV_SPEC> {
    ...
    #[inline(always)]
    #[must_use]
    pub fn cam_conv_protocol_mode(
        &mut self,
    ) -> CAM_CONV_PROTOCOL_MODE_W<CAM_RGB_YUV_SPEC> {
        CAM_CONV_PROTOCOL_MODE_W::new(self, 26)
    }
    ///Bit 27 - Configure color range for output data. 0: limited color range. 1: full color range.
    #[inline(always)]
    #[must_use]
    pub fn cam_conv_data_out_mode(
        &mut self,
    ) -> CAM_CONV_DATA_OUT_MODE_W<CAM_RGB_YUV_SPEC> {
        CAM_CONV_DATA_OUT_MODE_W::new(self, 27)
    }
    ///Bit 28 - Configure color range for input data. 0: limited color range. 1: full color range.
    #[inline(always)]
    #[must_use]
    pub fn cam_conv_data_in_mode(
        &mut self,
    ) -> CAM_CONV_DATA_IN_MODE_W<CAM_RGB_YUV_SPEC> {
        CAM_CONV_DATA_IN_MODE_W::new(self, 28)
    }
    ///Bit 29 - 0: 16-bit mode. 1: 8-bit mode.
    #[inline(always)]
    #[must_use]
    pub fn cam_conv_mode_8bits_on(
        &mut self,
    ) -> CAM_CONV_MODE_8BITS_ON_W<CAM_RGB_YUV_SPEC> {
