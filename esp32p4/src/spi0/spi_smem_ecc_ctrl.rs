...
impl W {
    ...
    pub fn spi_smem_ecc_err_int_en(
        &mut self,
    ) -> SPI_SMEM_ECC_ERR_INT_EN_W<SPI_SMEM_ECC_CTRL_SPEC> {
        SPI_SMEM_ECC_ERR_INT_EN_W::new(self, 17)
    }
    ///Bits 18:19 - Set the page size of the external RAM accessed by MSPI. 0: 256 bytes. 1: 512 bytes. 2: 1024 bytes. 3: 2048 bytes.
    #[inline(always)]
    #[must_use]
    pub fn spi_smem_page_size(
        &mut self,
    ) -> SPI_SMEM_PAGE_SIZE_W<SPI_SMEM_ECC_CTRL_SPEC> {
        SPI_SMEM_PAGE_SIZE_W::new(self, 18)
    }
    ///Bit 20 - Set this bit to enable MSPI ECC address conversion, no matter MSPI accesses to the ECC region or non-ECC region of external RAM. If there is no ECC region in external RAM, this bit should be 0. Otherwise, this bit should be 1.
    #[inline(always)]
    #[must_use]
    pub fn spi_smem_ecc_addr_en(
        &mut self,
    ) -> SPI_SMEM_ECC_ADDR_EN_W<SPI_SMEM_ECC_CTRL_SPEC> {
