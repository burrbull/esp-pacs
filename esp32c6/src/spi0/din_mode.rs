...
pub enum IN_MODE {
    ...
    NoDelay = 0,
    #[doc = "1: Input at the (DINx_NUM + 1)th falling edge of clk_spi_mst"]
    MstFalling = 1,
    #[doc = "2: Input at the (DINx_NUM + 1)th rising edge of clk_hclk plus one clk_spi_mst rising edge cycle"]
    HclkPlusMstRising = 2,
    #[doc = "3: Input at the (DINx_NUM + 1)th rising edge of clk_hclk plus one clk_spi_mst falling edge cycle"]
    HclkPlusMstFalling = 3,
}
...
impl DIN0_MODE_R {
    ...
    #[inline(always)]
    pub const fn variant(&self) -> Option<IN_MODE> {
        match self.bits {
            0 => Some(IN_MODE::NoDelay),
            1 => Some(IN_MODE::MstFalling),
            2 => Some(IN_MODE::HclkPlusMstRising),
            3 => Some(IN_MODE::HclkPlusMstFalling),
            _ => None,
        }
    }
    ...
    #[doc = "Input at the (DINx_NUM + 1)th falling edge of clk_spi_mst"]
    #[inline(always)]
    pub fn is_mst_falling(&self) -> bool {
        *self == IN_MODE::MstFalling
    }
    #[doc = "Input at the (DINx_NUM + 1)th rising edge of clk_hclk plus one clk_spi_mst rising edge cycle"]
    #[inline(always)]
    pub fn is_hclk_plus_mst_rising(&self) -> bool {
        *self == IN_MODE::HclkPlusMstRising
    }
    #[doc = "Input at the (DINx_NUM + 1)th rising edge of clk_hclk plus one clk_spi_mst falling edge cycle"]
    #[inline(always)]
    pub fn is_hclk_plus_mst_falling(&self) -> bool {
        *self == IN_MODE::HclkPlusMstFalling
    }
}
#[doc = "Field `DIN0_MODE` writer - Configure the input mode for FSPID signal. Can be configured in CONF state"]
pub type DIN0_MODE_W<'a, REG> = crate::FieldWriter<'a, REG, 3, IN_MODE>;
    ...
    REG::Ux: From<u8>,
{
    ...
    #[doc = "Input at the (DINx_NUM + 1)th falling edge of clk_spi_mst"]
    #[inline(always)]
    pub fn mst_falling(self) -> &'a mut crate::W<REG> {
        self.variant(IN_MODE::MstFalling)
    }
    #[doc = "Input at the (DINx_NUM + 1)th rising edge of clk_hclk plus one clk_spi_mst rising edge cycle"]
    #[inline(always)]
    pub fn hclk_plus_mst_rising(self) -> &'a mut crate::W<REG> {
        self.variant(IN_MODE::HclkPlusMstRising)
    }
    #[doc = "Input at the (DINx_NUM + 1)th rising edge of clk_hclk plus one clk_spi_mst falling edge cycle"]
    #[inline(always)]
    pub fn hclk_plus_mst_falling(self) -> &'a mut crate::W<REG> {
        self.variant(IN_MODE::HclkPlusMstFalling)
    }
}
...
impl R {
    #[doc = "Bits 0:2 - Configure the input mode for FSPID signal. Can be configured in CONF state"]
    #[inline(always)]
    pub fn din0_mode(&self) -> DIN0_MODE_R {
        DIN0_MODE_R::new((self.bits & 7) as u8)
    }
    #[doc = "Bits 3:5 - Configure the input mode for FSPIQ signal. Can be configured in CONF state"]
    #[inline(always)]
    pub fn din1_mode(&self) -> DIN1_MODE_R {
        DIN1_MODE_R::new(((self.bits >> 3) & 7) as u8)
    }
    #[doc = "Bits 6:8 - Configure the input mode for FSPIWP signal. Can be configured in CONF state"]
    #[inline(always)]
    pub fn din2_mode(&self) -> DIN2_MODE_R {
        DIN2_MODE_R::new(((self.bits >> 6) & 7) as u8)
    }
    #[doc = "Bits 9:11 - Configure the input mode for FSPIHF signal. Can be configured in CONF state"]
    #[inline(always)]
